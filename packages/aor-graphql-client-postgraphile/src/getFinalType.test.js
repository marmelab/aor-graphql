...
describe('getFinalType', () => {
    ...
    it('returns the correct type for NON_NULL types', () => {
        expect(getFinalType({ kind: TypeKind.NON_NULL, ofType: { name: 'foo', kind: TypeKind.SCALAR } })).toEqual({
            name: 'foo',
            kind: TypeKind.SCALAR,
        });
    });
    it('returns the correct type for LIST types', () => {
        expect(getFinalType({ kind: TypeKind.LIST, ofType: { name: 'foo', kind: TypeKind.SCALAR } })).toEqual({
            name: 'foo',
            kind: TypeKind.SCALAR,
        });
    });
