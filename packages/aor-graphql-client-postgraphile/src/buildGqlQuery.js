import { GET_LIST, GET_MANY, GET_MANY_REFERENCE, CREATE, DELETE, QUERY_TYPES } from 'aor-graphql-client/lib/constants';
...
import { UPDATE } from '../../aor-graphql-client/src/constants';
...
export default introspectionResults => (resource, aorFetchType, queryType, variables) => {
    ...
    if (aorFetchType === CREATE || aorFetchType === UPDATE) {
        const resourceLowerCased = `${resource.type.name.substring(0,1).toLowerCase()}${resource.type.name.substring(1)}`;
