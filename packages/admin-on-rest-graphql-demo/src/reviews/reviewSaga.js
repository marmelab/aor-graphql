...
export default function* reviewSaga() {
    yield [
        ...
        takeEvery(REVIEW_APPROVE_FAILURE, function* ({ error }) {
            yield put(showNotification('resources.Review.notification.approved_error', 'warning'));
            console.error(error);
        }),
        takeEvery(REVIEW_REJECT_SUCCESS, function* () {
            yield put(showNotification('resources.Review.notification.rejected_success'));
            yield put(push('/Review'));
        }),
